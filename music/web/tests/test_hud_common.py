...
def test_the_toast_and_panel_clear_the_dock():
    css = _text(SHEET)
    assert "body:has(.hud-dock) .toast { bottom: calc(18px + var(--cc-dock-h, 0px)); }" in css
    assert "body:has(.hud-dock) .mi-panel { bottom: var(--cc-dock-h, 0px); }" in css
