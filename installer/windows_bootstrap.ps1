...
# Drive-mapping primitives: Invoke-AtUserIntegrity, Invoke-MappingCommand,
# ConvertFrom-CanonicalPrefix, ConvertFrom-DriveMapReport, Get-DriveMapping,
# Test-IsElevated. They moved into installer/drive_mapping.ps1 on 2026-08-28
# (OPS-8 / UX-23) because windows_uninstall.ps1 needs the SAME classification
# and had drifted into a weaker one that read "could not tell" as "ours".
#
# A missing library is fatal HERE rather than a NotRecognized error 800 lines
# down: every remaining caller either mounts or tears down a drive letter, and
# a teardown with no ownership check is how the base rig's real NAS mapping
# gets deleted (B21).
$DriveMappingLib = Join-Path $PSScriptRoot "drive_mapping.ps1"
if (-not (Test-Path -LiteralPath $DriveMappingLib)) {
    Write-Host "[ccsync] ERROR: drive_mapping.ps1 is missing from $PSScriptRoot. It ships beside this script in the editor package; download the package again (do not copy windows_bootstrap.ps1 on its own)." -ForegroundColor Red
    exit 1
}
. $DriveMappingLib
