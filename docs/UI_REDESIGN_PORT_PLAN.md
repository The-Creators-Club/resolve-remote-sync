...
|---|---|---|
| `fleet.html`, `GET /`. Partials: `sidebar`, `notices` (60 s), `fleet_grid` (15 s, includes `collector_health`), `plan_changes` (60 s), `transfers` (2 s), `queue_section`/`my_queue` (10 s), `project_roots` (30 s) + `project_roots_browse`, `#fleet-diagnostics`. | `home.html` | Largest change. Bench: head keys **Stop the fleet** (UI, the route exists on Users) and **Check now** (BACKEND). A stale alert line. Four **readouts** (online, moving, problems, in sync; UI, aggregated in the view builder). **problems.log** (notices restyled, plus **dismiss all**, BACKEND). **computers** as fixed-height rows: three lane meters **with a percent per lane** (BACKEND, lane chips carry no percent), issues listed inline worst first (UI, from the chips behind today's `[ DETAILS ]`), version with "N behind" (BACKEND, count of releases), and collector age in the bar (UI). A transfers window of only what is moving, and plan_changes without the UNDO column. **Lost on the way:** the project cards overview, about ten fleet banners (`fleet_grid.html` lines 37-130), everything behind `[ DETAILS ]`, `[ RESUME ]`, `[ ASK THIS COMPUTER WHY ]`, `[ READ THE ANSWER ]`, LOST COMPUTERS + `[ FORGET ]`, the queued and history halves of the transfers window, the SYNC QUEUE panel, PROJECT ROOTS (moved to project), plan-change `[ UNDO ]`. Each needs a home before home ports (section 5.1). |
...
| `admin_assignments.html` + `static/assignments.js` (no htmx) | `sync-plans.html` | The computer select is shown before a person is chosen, filled client-side (UI, the page must emit the person-to-machines map it already builds in `_picker`). Free space and proxy size become visible text (UI, today they are `data-*` only). Dialog confirm. |
| `admin_packages.html` + `partials/admin_packages.html` (30 s) + `partials/admin_dashboard_update.html` (load only) + `dashboard_update.js`, `confirms.js` | `packages.html` | **UPDATE ALL** panel (BACKEND, section 5.3). The "OTHER VERSIONS HELD" list is replaced by a **roll back to…** dropdown on each served row (UI over `POST packages/current`). Its "goes to the trash for 30 days" wording is true (both delete routes call `_trash_package_file`); the live confirm undersells it. The dashboard's own rollback becomes a select plus a key, and the apply-older and image-rollback lists stay two separate controls (a JS change with a test per path, section 5.3; `dashboard_update.js` reads `data-dashupd-*` from `evt.target` itself, section 3.3). |
...
| `admin_alerts.html` + `partials/admin_alerts.html` | `alerts.html` | UI. Head keys; four windows, one save. Severity per kind is UI if `ALERT_KINDS` carries it (check). |
...
- **theme-common goes into `cc/terminal.css` byte for byte in phase 0**
  ...
  carried: theme-common is the one definition. In phase 8 the `"dashboard"`
...
- **Static JS is shared by both variants** (the overlay is templates only).
  ...
  `copy_value.js`, `setup.js`, `assignments.js`, `dashboard_update.js`)
...
- **Contrast** (computed on `#070403`): `--muted` 5.45:1, `--text-2`
  ...
  about 5:1), or black text on red. `--red-deep` (scrollbar thumb, tree
...
- **Fields at 16 px on phones, 44 px hit boxes on touch** (wave 3). The bench
  ...
  `mobile.css:249`). `cc/phone.css` carries `input, select, textarea, .inp,
  .sel, .area { font-size: 16px }` on the phone query, and a
...
- **The HUD must actually stick on classic pages and in the SPAs** (wave 4).
  ...
  `#app-header` in the SPAs. Making it stick exposes the classic sticky
...
- **The installed app pays the safe-area insets in hud-common** (wave 4).
  ...
  a padding alone would crush a border-box 58 px bar), pads the HUD and dock
  sides with `max(24px, env(safe-area-inset-left, 0px))` and the right-hand
  twin, and the `scroll-padding-top` calc includes the top inset. Phase 1:
...
- Ship woff2 in `dashboard/static/fonts/`: JetBrains Mono 400/500/700 and
  ...
  rather than a fallback, or the meters misalign.
...
- `docs/legal/THIRD_PARTY_NOTICES.md`: add both fonts to the hand-maintained
  block in `tools/gen_notices.py`, with the OFL text. `tools/check_licenses.py`
  reads lockfiles only, so it will not catch this. The review must.
...
- The grain animates at 6 frames a second behind every page, forever. On a
  ...
  `prefers-reduced-motion` query also turns off the grain and the HUD's
  `backdrop-filter` blur (wave 3), not only the animation.
- `backdrop-filter: blur()` on `.hud` makes the HUD the containing block of
  any `position: fixed` descendant. The dock and the drawer must not be
...
`question`). Decision: `cc/project_detail.html` keeps that handler and its
...
- One `htmx:confirm` handler in `cc/cc.js`. htmx 1.9.12 fires
  ...
  `[popover]` or `<dialog>` in `cc/shell.html`) and stores the event.
- While it is open, `busy()` holds EVERY poll: a document-level check
  (`document.querySelector('#cc-confirm:popover-open, #cc-confirm[open]')`)
...
- **The user menu links to the look switch** (wave 3). `#hud-user` (desktop)
  ...
  `next` or `Referer`, validated like `_safe_next`. **Only admins can opt
  into unreviewed templates** (wave 4; D14 and "the owner sees it before
  editors"): `variant=cc` answers 403 to a non-admin session unless the
  site flag `ui_preview_allowed` is on (off in the vendor build); an editor
  sees only the `classic` and `site` links, in the HUD menu and on
  `/account`. Tests: an editor's `?variant=cc` is 403; `/account` shows the
  cc link only to admins.
- The stamp and its 30 s poll. Keep it as `#topbar-stamp` in the HUD meta.
  ...
  tap, and is repeated in `#hud-more`), and nothing when all is well. Phase
...
| Item | Tag | Note |
|---|---|---|
| Readouts (online x/y, moving GB, problems open, in sync x/y) | UI | Aggregate in the `/` and `/partials/fleet` view builders from data they already load. Poll with the grid (oob into the readouts). |
...
| Computers: fixed three-line rows, issues inline worst first | UI | Issues come from the chips behind today's `[ DETAILS ]`. Pick one variant (D11). A row scrolls inside its slot, per the owner's round 2 rule. |
...
| Version "N behind" | BACKEND (small) | The count of releases between two versions, from the package store. |
...
| Project tree find box, `/` key, "n of m ticked" | UI | Client-side filter. The tree is re-rendered every 30 s (poll), so the filter text must be re-applied after swap (keep it outside the swap target, the 3.1 rule). |
| Size per project in the tree | BACKEND (small) | Not in the sidebar view. |
| What the bench drops from home | UI (a decision) | Fleet banners, `[ DETAILS ]` content, RESUME / ASK WHY / READ THE ANSWER, lost computers + FORGET, the sync queue, transfers queued/history, plan-change UNDO, the project cards overview. **None may be lost.** Recommended homes: banners go in the alert line slot (stacked, each foldable); per-computer details and actions go in a row expander (a nested `<details data-key>` under the row, which the round 2 rules allow), and home keeps an empty `#fleet-diagnostics` window outside the grid body for READ THE ANSWER to fill (otherwise the button raises `htmx:targetError` and does nothing); lost computers is its own window on home; the sync queue goes into the account page (it is the person's queue) and stays on home for editors; plan-change UNDO stays in the plan_changes window. The project cards overview is dropped only if the owner says so (D7). |
...
| Roll back to… dropdown per served row | UI | POST `packages/current` exists; previously-current builds skip the soak via `ever_current`. The unsigned typed override stays. |
...
| R20 | **Customer names or site specifics in code.** The bench says "CC SYNC / the creators club", `TPE`, `owen@admin`. | Brand from `brand_org` (site data), product name as the fallback; no clock (D4); user from the session. |
...
- **The per-browser cookie is three-way and beats the setting:**
  ...
  users only; it changes only that browser; a GET is fine because it writes
  nothing server-side). `variant=cc` is ADMIN-only unless the
  dashboard-only site setting `ui_preview_allowed` is on (off in the vendor
  build; never in `/api/v1/site`'s `features`), and a cookie `cc` presented
  by a non-admin session is read as absent (wave 4, 3.4).
- `_render` decides the group set per request in this order: the `X-CC-UI`
  header on an htmx request (the set of the page that asked, R23), then the
  cookie, then the setting. A page and its partials therefore agree even
  across a change of setting. `X-CC-UI: all` serves every `cc/` template
...
- Every full-page response, the `/partials/topbar` response (which every
  ...
  (wave 4: a comma list, `chrome,apps`, `chrome` or empty, so a SPA's
  first-paint script also knows whether the HUD is coming), with an
  explicit `Path=/` (set under `/admin/...` without one, the SPAs could not
  read it). **The client-side rewrite uses the same path** (wave 4): a
  ...
  (the phone height in the phone query). The injected marker corrects a
  stale cookie as below. The SPAs pick their body variant from it before first
...
- **Plumbing** (phase 0): `ui_terminal_groups` is a site setting read by the
  dashboard only (a `KEYS` entry with validation against the group names, a
...
- **New-named terminal partials** (wave 3). A terminal partial with no
  ...
  `home_queue`, `computer_answer`, `projects_tree`, `halt_line`,
  `hint_sheet`, `person_queue`, `health_notices`, `health_collector`,
  `health_diagnostics`) lives only under `templates/cc/partials/` and is
...
- Rollback at any phase: take the group off the setting (R24), and every
  ...
  introduced an enabled group is preceded by `tools/ui_variant.py off`
  (R23's runbook line).
- The overlay is temporary. Phase 8 deletes classic, moves `cc/` up, and
  removes the loader, the setting, the group-set header and the cookie
  route. The generation token and its reload line stay for good (R23).
...
- Only the dashboard image (or bundle) changes, in every phase. The SPAs are
  inside it. There is no wire change and no schema change in any port
...
| 6 Apps | 4-6 | Three SPAs + Cards landing; ytdl app.js is 3,000+ lines. |
| 7 Copy sweep | 2-3 | Alerts copy is emailed: review sentence by sentence. |
| 8 Default and delete | 1-2 | Plus the soak. |
| **Total** | **27-39** | The BACKEND tickets (section 5) are not in these numbers: user page 3-5, UPDATE ALL 4-6 (with its own design review), check now 1, dismiss all 1, the small view fields 1-2 together, collector route 0.5, who_answers 0.5. |
