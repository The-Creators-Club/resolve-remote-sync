...
def _taken_on_disk(name):
    ...
    """
    try:
        return config.safe_join(config.share_root(), name).exists()
...
def mark_uploaded(conn, batch, item, *, size=None):
    ...
    if not item['track_id'] or not item['dest_name']:
        raise HTTPException(409, {
            ...
            'reason': 'unindexed'})
    try:
        path = config.resolve_path(batch['share'] or config.SHARE,
                                   item['dest_name'])
    ...
    try:
        ...
        raise HTTPException(409, {
            ...
            'reason': 'not_uploaded', 'rel_path': item['dest_name']}) from None
    ...
    return {'ok': True, 'live': True, 'rel_path': item['dest_name'],
            'bytes': actual, **counts}
