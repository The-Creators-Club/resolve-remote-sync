# Bug hunt 2026-08-11 — the 82-finding ledger, as fixed
...
**Resolution status (2026-08-11, same day):** every entry below was fixed the
day the hunt landed, in a single orchestrated pass — eight Opus agents over
disjoint file territories (companion sync core / sequencer+delete-protection /
UI / media pipeline; dashboard; b-roll; music; ops), verified against source
by the orchestrator, with regression tests per entry. The same pass also
implemented `docs/delete-protection-ignoredelete.md` (plus a NAS-side
retrofit the doc missed: `ignoreDelete` joined the dashboard collector's
repairable folder drift, since the companion's per-turn PATCH only reaches
the editor's own Syncthing) and fixed the 45-finding ytdl ledger
(`docs/youtube_dlp_bugs.md`) with three more agents. Full test sweep after:
all ten suites green (the one flake was a pre-existing timing-sensitive
express-lane test under CPU load; 5x green idle).

What did NOT get a wholesale fix, or diverged deliberately, lives in
KNOWN_BUGS.md as the still-open ledger. Notable deliberate divergences from
the fixes as written below:

- **SYNC-4:** the except-arm redaction shipped, but the password still rides
  `net use`'s argv — moving it to stdin conflicts with the load-bearing
  `stdin=DEVNULL` prompt-hang fix (error 1223). Design change, still open.
- **MED-4:** `trackIndex` + `GetStart()` verification shipped, but
  `mediaType` is deliberately NOT sent — pinning it to one stream would drop
  a b-roll clip's nat sound, the same silent-wrongness class as MED-1.
  Pinned by `test_the_append_does_not_pin_a_mediatype`.
- **BROLL-1/2:** code + schema (migration 009, five `sprite_*` columns)
  shipped, and the base-rig archive was regenerated the same day
  (7,118 sheets rebuilt with recorded geometry; 428 rows remain on the
  legacy fallback — 390 with no local proxy, 38 unspritable error/degenerate
  rows that never had a sheet).
- **BROLL-16:** the fix OVERRODE a documented decision — the old test pinned
  case-sensitivity as deliberate for a NAS that "holds `youtube` and
  `Youtube` as distinct folders", but every configured share root is a
  case-insensitive Windows drive; review if a NAS-rooted share ever appears.
- **OPS-1:** ship now builds through `tools\release.ps1` (parity, editable
  install, companion+dashboard suites, PyInstaller, manifest) and publishes
  that artifact WITHOUT `-RebuildExe`.

The original hunt text follows, unedited, for the write-ups and file:line
references (as of commit 4075b3c).
...
- **macOS runtime validation backlog.** `installer/MACOS_FIRST_RUN.md` §A7–H is
  ...
  FSEvents stream on the Mac editor's external SSD still needs someone at the machine
