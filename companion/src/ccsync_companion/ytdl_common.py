# VENDORED VERBATIM from ytdl/web/ytdlweb/ytdl_common.py -- DO NOT EDIT HERE.
# Edit the server's copy and re-copy it in BELOW THE MARKER LINE at the end of
# this header; everything under that line is byte-identical to the source on
# purpose, so the two trees can be compared mechanically -- and since 2026-08-14
# they ARE: tools/release.ps1 strips this header, byte-compares the rest against
# the source and REFUSES to build on a mismatch (docs/YTDL_LOCAL_DOWNLOAD.md §5,
# the same mechanism it already uses to refuse on installer version drift), and
# server/tests/test_cross_component.py pins the same comparison in the suites.
#
# Why a copy and not a shared package: the companion is a frozen PyInstaller
# build with no ytdlweb, no FastAPI and no yt-dlp library in it, and the
# dashboard container has no ccsync_companion. Vendoring is how the installer's
# parity files already cross that gap.
#
