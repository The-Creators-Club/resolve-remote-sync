...
6. **Crunch per item** (checkpointed): browser preview (540p at the time of writing; 1080p since 2026-09-17, `BROLL_PROXY_TIERS_PLAN.md` phase 1, with a post-encode frame count) (NVENC → CPU retry) → sprite + poster from the proxy → scene-detect + frames → describe (vendored `local_vlm.describe_video` against a companion-managed llama-server) → `POST …/items/{uid}/result` (server writes segments, computes `search_norm`, status `indexed`).
