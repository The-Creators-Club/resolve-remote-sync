...
1. You should have received this folder (the new CC_Sync package) from Alex —
   either as a download link or copied onto your machine. Open **PowerShell**
   in this folder (in Explorer: File → Open Windows PowerShell, or
...
- **One-click updates from now on.** When Alex publishes a new version, your
  tray icon pops a notification and the tray menu grows an
...
- **Tray icon red, or anything confusing:** message Alex with a screenshot
  of the tray menu. The log at `%USERPROFILE%\.ccsync\companion.log`
