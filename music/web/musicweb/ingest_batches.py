...
# music-1 (2026-09-11): how many `name (n).ext` candidates one allocation may
# mint before it calls the library broken. A drop of MAX_BATCH_ITEMS tracks all
# called `theme.wav` is well under this; a thousand is not a collision, it is a
# predicate answering TAKEN because it cannot answer at all.
_MAX_NAME_CANDIDATES = 1000


def retry_detail(text, reason):
    """A refusal body that says, in the payload, that it is worth retrying.

    music-1/music-2 (2026-09-11). The companion's `_post_result` used to branch
    on the status alone and end the track, so the two refusals the server emits
    meaning "ask again" (this, and the 409 `name_race`) killed a whole album
    drop when a bind mount blipped for thirty seconds. `reason` is the machine
    half and `retry` is the flag a companion that knows no reason codes can
    still read; the text is what an editor sees on the page.
    """
    return {'detail': text, 'reason': reason, 'retry': True}
...
def allocate_name(conn, orig_name, transcoded=None, reserved=None):
    ...
    if not ok:
        raise HTTPException(503, retry_detail(why, 'share_not_ready'))
    reserved = reserved if reserved is not None else set()
    name = final_name_for(orig_name, transcoded)
    stem, ext = Path(name).stem, Path(name).suffix
    candidate = name
    # music-1 (2026-09-11): a CEILING, and "could not tell" is not "taken".
    # Both predicates below answer TAKEN on error, which is the harmless
    # direction for a transient fault and an infinite tight loop for a
    # persistent one: an EACCES or EIO out of Path.exists() (a bind mount this
    # uid can stat but not traverse - it propagates, pathlib swallows ENOENT
    # only), or a database that stays locked or malformed, made every candidate
    # taken for ever. It ran on a uvicorn threadpool thread at 100% CPU, the
    # companion's HTTP timeout retried it, and only a container restart cleared
    # the pile. Now the uncertainty is reported as a 503 the companion retries
    # later, and 1,000 collisions on one stem is a refusal rather than a loop.
    for i in range(2, _MAX_NAME_CANDIDATES + 2):
        free, unsure = _candidate_free(conn, candidate, reserved)
        if unsure:
            raise HTTPException(503, retry_detail(
                'the library could not say whether a filename is free right '
                'now (%s). Nothing was written: this is retried shortly.' % unsure,
                'library_unavailable'))
        if free:
    ...
    raise HTTPException(503, retry_detail(
        'the library already holds %d files named like "%s", which is not a '
        'library it can add to safely. Nothing was written.'
        % (_MAX_NAME_CANDIDATES, name), 'too_many_collisions'))
...
def _candidate_free(conn, name, reserved):
    """Is this exact name free? -> (free, could_not_tell_reason|None).

    music-1 (2026-09-11): the one place the collision loop asks its question,
    and the reason it can now stop. "Free", "taken" and "I cannot tell" are
    three answers, and only the first two are worth minting another candidate
    for; the third is a 503 at the caller.

    It replaced `_taken_on_disk` / `_taken_by_a_track` (bug-hunt-2026-09-03
    music-3), whose whole contract was "never raises, and an error counts as
    TAKEN". That is the harmless direction for a caller that asks about ONE
    name and the wrong one for the caller that loops, and this was their only
    caller, so keeping both shapes would have left a predicate nothing uses
    for the next reader to trust (music-4's lesson, the same day).
    """
    if name.lower() in reserved:
        return False, None
    taken, unsure = _disk_state(name)
    if unsure:
        return False, unsure
    if taken:
        return False, None
    taken, unsure = _track_state(conn, name)
    if unsure:
        return False, unsure
    return not taken, None


def _disk_state(name):
    """(taken, could_not_tell_reason|None) for the library directory."""
    for spelling in _spellings(name):
        try:
            ...
            # A name that cannot be joined is one nothing could have landed
            # under either, so it is a collision to be stepped around, not an
            # unreadable share.
            return True, None
...
def _track_state(conn, name):
    """(taken, could_not_tell_reason|None) for the `tracks` table."""
    try:
        for spelling in _spellings(name):
...
def retry_failed(conn, batch):
    """Put this batch's failed tracks back in the queue. Returns what it did.

    music-2 (2026-09-11), the music half of BROLL-18. `done_with_errors - 3
    failed` was the end of the road here: b-roll has had a retry route and a
    button since 2026-09-04 and music never grew one, so a drop that met a
    momentary refusal (a bind mount that blipped, a name race, a 503 out of
    `allocate_name`) died with the audio still staged on the editor's machine
    and the only way back was to drop the files again by hand.

    The same three limits b-roll's has, for the same reasons:

      * only `failed` items move. Everything in ITEM_TERMINAL is terminal on
        purpose (_check_transition), and a track already in the library that
        somebody has cut with must not be re-indexed underneath them.
      * `track_id` and `dest_name` STAY. The name is already allocated and
        `write_item_result` updates in place when a track_id is set, so a
        retry re-uses the slot rather than minting `theme (2).mp3`.
      * nothing is dispatched here. The batch goes back to `queued` and a
        companion claims it exactly as it claims a new one - the browser never
    ...
    with conn:
        ...
        conn.execute(
            ...
            'error = NULL, updated_at = ? WHERE batch_uid = ? AND '
            "state = 'failed'", (now, batch['uid']))
        if item_uids:
            conn.execute(
    ...
    if item_uids:
        log.info('music ingest: batch %s retrying %d failed track(s)',
                 batch['uid'], len(item_uids))
    fresh = get_batch(conn, batch['uid'])
    return {'ok': True, 'retried': len(item_uids), 'state': fresh['state'],
