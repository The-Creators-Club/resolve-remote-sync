...
def test_health_partials_serve_terminal_markup(env, ui_variant, route, marker):
    ...
    if route == "/partials/health-collector":
        # LG-17 carried from the classic panel (2026-09-25): when the
        # retention pass last ran is one line (the app's start may have run
        # it already, so either wording).
