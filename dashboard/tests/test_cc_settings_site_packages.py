...
def test_site_page_renders_in_its_look(ui_variant, env):
    ...
    # LG-1 / LG-4 carried from the classic page (2026-09-25): the four
    # telemetry switches ride #settings-form with the classic names and the
    # data-initial the save-time question compares against; the transport
    # line is present for site_settings.js to fill.
    for cat in ("resolve_project", "local_manifest", "media_tree", "input_idle"):
        assert f"telemetry.{cat}" in names, cat
        box = next(k for k in settings_form["kids"] if k["attrs"].get("name") == f"telemetry.{cat}")
        assert box["attrs"].get("data-telemetry") == cat
        assert box["attrs"].get("data-initial") in ("0", "1")
        assert box["attrs"].get("data-label")
    assert 'id="telemetry"' in html
    assert "site-transport-line" in ids
...
def test_cc_site_settings_js_writes_only_the_classic_routes():
    ...
    # LG-1 / LG-4 (2026-09-25): save, import and undo all name what a
    # telemetry switch-off deletes; the transport line is filled.
    assert "telemetryOffQuestion(switchingOff)" in src
    assert "preview.telemetry_off" in src
    assert "latest.telemetry_off" in src
    assert "markTelemetrySaved(form)" in src
    assert "report_via_counts" in src and "site-transport-line" in src
