...
FOUR DECISIONS, each of which is a whole class of bug or a policy:
...
class Runner:
    ...
    def run(self, prompt: str, model: str = "", timeout: float = DEFAULT_TIMEOUT,
            think: bool = True, json_out: str = "") -> dict[str, Any]:
    ...
    def _sdk(self, prompt: str, model: str, timeout: float) -> str:
        key = self._key()
        if not key:
    ...
    def _cli(self, prompt: str, timeout: float) -> str:
        path = self._cli_path()
        if not path:
