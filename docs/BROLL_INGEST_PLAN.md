...
6. **Crunch per item** (checkpointed): browser preview (540p at the time of writing; 1080p since 2026-09-17, `BROLL_PROXY_TIERS_PLAN.md` phase 1, with a post-encode frame count) (NVENC → CPU retry) → **editing proxy** `Proxy/<stem>.mov` when the original is heavier than edit-weight (`BROLL_PROXY_TIERS_PLAN.md` phase 2: `own_proxy_cmd`, the same NVENC → CPU retry and the same frame-count verify; an edit-weight original and a BRAW record `edit_proxy: null` with their reason on the item's checkpoint and encode nothing) → sprite + poster from the proxy → scene-detect + frames → describe (vendored `local_vlm.describe_video` against a companion-managed llama-server) → `POST …/items/{uid}/result` (server writes segments, computes `search_norm`, status `indexed`).
7. **Upload** (separate thread, starts as each item finishes; pause toggle): `rclone copyto` up: `posters/{id}.jpg`, `sprites/{id}.jpg`, `<folder>/Proxy/<stem>.mp4`, then `<folder>/Proxy/<stem>.mov` when there is one, originals last; `--stats 1s` JSON progress; then `POST …/items/{uid}/uploaded` → server stats each file (including a **declared** `edit_proxy_rel`, refusing to flip live if it is missing or zero bytes), sets `archive_path`, `original_path`, status → `live`, bumps search generation. The finished preview is also placed at the local mirror path (`<local_root>/Assets/B-roll Archive/<archive_path>`) so "Send to Resolve" needs no fetch on this machine.
