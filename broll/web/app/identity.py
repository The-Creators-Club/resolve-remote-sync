...
# 3.1). The b-roll ingest fleet routes authenticate exactly as ytdl's do -- the
# shared DASH_REPORT_TOKEN proves "a fleet machine" and NOTHING about which, so
# the editor's name arrives as the dashboard's signed identity token and is
# verified before it is believed (H5, COMMERCIAL_READINESS.md item 7). That
# verifier is this file, and it must be the SAME one: an ingest route that
# accepted a token shape ytdl rejects, or rejected one ytdl accepts, is two
# answers to "who is this companion" and one of them is wrong.
#
# broll/web cannot import ytdlweb. It is deployed as its own tree on PYTHONPATH
# and imported as top-level `app`; ytdl is a separate, feature-gated tree that a
# site may not have shipped at all (site.toml [features] youtube_download), and
# b-roll ingest must not stop working because a customer turned the YouTube
# downloader off.
#
