...
def _declared_machine(x_ccsync_machine: str = Header(default=None),
                      x_ccsync_machine_pct: str = Header(default=None)):
    ...
    companion 0.9.79 (broll_ingest.FleetClient, which music ingest shares) it
    sends the name percent-encoded in `X-CCSync-Machine-Pct` instead. Reading
    only the plain header left such a machine with no machine check (the
    documented older-companion degrade). The plain header wins when both are
    present, so an older companion and a Latin-1 name behave exactly as
    before; a companion that sends neither still gets None and the
    editor/lease/cancel checks. Same rule as broll/web's routes_fleet.
    """
    if x_ccsync_machine:
    ...
    if x_ccsync_machine_pct:
        ...
        return urllib.parse.unquote(x_ccsync_machine_pct) or None
    return None
...
def heartbeat(uid: str, body: HeartbeatIn, editor: str = Depends(require_fleet_caller),
              x_ccsync_machine: str = Depends(_declared_machine)):
...
def release(uid: str, body: ReleaseIn, editor: str = Depends(require_fleet_caller),
    ...
    if body.state == 'cancelled':
        ...
        # bug-broll-4, music twin (Fable round, 2026-09-25): "this editor's
        # batch" was the whole check, so a machine whose lease had expired and
        # been TAKEN OVER by another of the same editor's machines could still
        # cancel it, and since bug-music-ytdl-4 a cancelled release runs
        # _drop_unlanded_tracks: the stale machine deleted the holder's tracks
        # rows (embedding, windows, peaks) while its audio was uploading. The
        # relaxation exists for a lease the dashboard expired, not one somebody
        # else now holds. A caller that names no machine keeps the old
        # behaviour; the companion's release treats this 410 as "the server
        ...
        if (x_ccsync_machine and batch['machine']
            ...
            raise HTTPException(410, {
                'detail': f'{batch["machine"]} holds this batch now',
