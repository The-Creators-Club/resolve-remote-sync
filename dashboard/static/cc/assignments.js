// The terminal look's copy of assignments.js (UI redesign port phase 4,
// 2026-09-25; plan 3.3, 5.3). Loaded only by cc/admin_assignments.html,
// never beside the classic file. Same writes, same confirms (native, wave 5),
// same column tools and copy-from. What differs: labels carry no brackets
// and are written into the key's `.t` span; the result notes are
// `.assign-note` (the terminal `.toast` is the shell's one toast); and the
// computer select is filled in the browser from #assign-machine-map when the
// person changes, so both pickers are visible from the start.
//
// Classic original's header follows.
...
(function () {
  ...
  // The computer picker (5.3): filled from the person-to-computers map the
  // page emits, so a person change never needs a round trip. The values and
  // the "-" / every-computer options are the classic form's own.
  (function picker() {
    var data = null;
    try {
      var el = document.getElementById("assign-machine-map");
      data = el ? JSON.parse(el.textContent || "{}") : null;
    } catch (err) { data = null; }
    var ed = document.getElementById("assign-editor");
    var mc = document.getElementById("assign-machine");
    if (!data || !ed || !mc) return;
    ed.addEventListener("change", function () {
      var opts = (data.map || {})[ed.value] || [];
      ...
      add("-", "choose a computer…");
      if (opts.length > 1) add(data.all, "every computer");
      opts.forEach(function (o) { add(o.value, o.label); });
      if (opts.length === 1) mc.value = opts[0].value;
      mc.disabled = !ed.value;
    });
  })();
  ...
  function toast(message, kind) {
    ...
    el.className = "assign-note " + (kind || "ok");
  ...
  function runProgress(btn, done, total) {
    ...
    if (btn.dataset.idleLabel === undefined) btn.dataset.idleLabel = label.textContent;
  ...
  function runDone(btn) {
    if (!btn) return;
    var label = btn.querySelector(".t") || btn;
    if (btn.dataset.idleLabel !== undefined) label.textContent = btn.dataset.idleLabel;
