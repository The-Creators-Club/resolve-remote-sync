...
| `DASH_SITE_AI_CLI_AUTO_UPDATE` | **F** | `"1"` keeps a wizard-installed Claude Code up to date unattended (`[features] ai_cli_auto_update`, CR-309). Off in the vendor build; inert without the row above. A `site_settings` row (Settings, or a pasted `site.toml`) beats it either way |
...
| `DASH_CARDS_TOKEN` | `""` | That server's own `CARDS_TOKEN`. **It never leaves this container**: the tunnel attaches it outbound, which is what takes it off the editor's machine. An upstream 401/403 is reported as "check DASH_CARDS_TOKEN", not as the caller's problem. With the page MOUNTED here (below) it is also the engine's own agent token, and the tunnel needs neither |
| `DASH_CARDS_ENABLED` | `""` | **`1` and nothing else** turns on the mounted Timeline Cards page at `/cards` (phase 3, `docs/TIMELINE-CARDS-INTO-CCSYNC.md` §7d). Anything else is off, and off is the shipped state. A `CARDS_SRC` in the environment counts as on -- that is the dev hook, and no deployment sets it |
| `DASH_CARDS_SRC` | `""` | The MulticamPipeline checkout `multicam_pipeline.cards` is imported from -- `/cards-app` in the container, shipped there by `install_dashboard_app.py` as a `git archive` export of ONE COMMIT of the repo holding `[timeline_cards] src` (2026-09-11; `--cards-commit <ref>` pins one, `CARDS_SRC` ships a directory as it stands, and the shipped tree carries a `DEPLOYED_COMMIT` marker naming the commit). **Not on `PYTHONPATH`**: another repo's tree is never in the image or in a code bundle, so `cards.py` appends this to `sys.path` itself. Blank = `/cards` is `disabled` naming this variable; a path that is not there = `absent` |
| `DASH_CARDS_VAULT_ROOT` | `""` | The vault as the CONTAINER sees it (`/vault`), where transcripts are read and every plan, note and translation cache is written. Blank = `/cards` says "no vault is mounted here". The bind mount and the chown it needs first are `docs/DOCKER.md`, "The Timeline Cards mounts" |
| `DASH_CARDS_ROOT` | `""` | The episode an engine falls back to when the pool is given no root (a dev run against one episode). Since 2026-09-14 the EPISODE COMES FROM THE URL -- `/cards/p/<slug>/`, minted per episode root -- so this is no longer "the episode the page opens on", and the page's own root menu (`POST /api/root`) is refused here: switching episode means going back to `/cards` and opening the other one, which leaves this one open behind you |
| `DASH_CARDS_ENGINES` | `2` | How many EPISODES may have an engine at once (`docs/CARDS_TWO_PROJECTS.md`). The third person is REFUSED with a sentence naming who is where, never evicted: `stop()` in the other repo sets a flag its library, tokens and translator threads do not read, so an eviction would leak three threads per episode. An engine holds one episode's clip resolutions, transcripts, token caches and peaks, so raise this only against a memory measurement -- the dashboard is what tells everyone whether their footage is syncing |
| `DASH_CARDS_PROJECT` | `""` | A `.cut.md` PROJECT FILE to serve instead of the agent's timeline. Blank = the timeline server, which is what the page has always been |
| `DASH_CARDS_DB_HOST` / `DASH_CARDS_DB_NAME` | `""` | Resolve's project library (PostgreSQL) and which library to open first. With them the page comes up with cards from the SAVED timeline before any agent has connected. **Not `localhost`** -- the project server is another container |
| `DASH_CARDS_DB_WRITE_ALLOW` | `""` | Comma-separated LIBRARY NAMES this server may WRITE (a conform built straight into Resolve's own database). Blank = no library write is possible, and the page says so rather than offering a route that cannot work |
| `DASH_CARDS_DB_BACKUPS` | `""` | Where the writer puts the row backup it takes before every write. Blank = `library_backups/` under the app's own `/data` |
| `DASH_CARDS_MEDIA_MAP` | `""` | How this container reaches the media an agent names: `P:/=/media`. **Useless without the `/media` bind mount and vice versa** -- either alone means the lane has no audio for any clip with no rendered WAV, which reads as a broken clip |
