"""The v7 COMPACT format: prompt builder, GBNF grammar, tolerant parser, and
the two things the model is no longer asked for (category, quality flags).

Why compact: the four-clip prototype (docs/local-indexing-options-2026-08-17.md
§2) showed a 4B model spending its budget on JSON punctuation, inventing
timecodes (246 s on an 88 s clip) and looping. Here the model gets frames
labelled F1..Fn with their timecodes IN THE TEXT and answers one line per
shot:

    S F1-F3 | terse description | obj;obj;obj | onscreen text or - // english or -
    T theme;theme

Segment times are LOOKED UP from the frame table (t_start = first frame's t,
t_end = last frame's t — the same convention v6 asks Haiku to use), the
category is assigned in code by nearest neighbour, and exposure flags come
from pixel statistics. `parse_compact` returns the exact dict shape
...
from common import HERE, seconds_to_tc  # noqa: F401  (sys.path side effect for broll_index)
from broll_index.claude_client import validate_contract  # noqa: E402

PROMPT_PATH = HERE / "prompt_v7_compact.md"
...
def build_grammar(n_frames: int, max_desc: int = 220, max_objs: int = 300,
    ...
    - a text field is EITHER the single "-" OR text that does not start with a
      dash/space: without that, "-" was just the first char of a free-text
      field and the model kept writing after it on the same line ("- // -
      → S F4-F6 | ..." was seen in the smoke test) — the grammar must make
      the newline the only continuation of a "-".
...
def parse_compact(text: str, frames: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], list[str]]:
    ...
    for s in shots:
        segments.append({
            ...
            # extra bookkeeping the contract ignores; handy for scoring
...
class CategoryAssigner:
    ...
    `.method` says which one ran — the report prints it.
    """
...
def pixel_quality_flags(paths: Sequence[Path], share: float = 0.3) -> tuple[list[str], list[dict]]:
    ...
    underexposed : >= `share` of NON-black frames have mean < 45 (a black frame is
                   content — v6 labels it — not an exposure fault)
