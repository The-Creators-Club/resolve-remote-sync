...
def test_the_panel_headings_use_the_red_dim_hairline():
    """Both idioms: the plain <h2> and the collapsible .panelhead that wears
    the [-]/[+] toggle. red-dim, not the grey --border used for row
    separators (2026-08-18)."""
    for selector in ("h2", ".panelhead"):
        assert "border-bottom: 1px dotted var(--red-dim)" in _rule(selector)


def test_the_collapsible_heading_is_still_a_red_terminal_label():
    body = _rule(".ptoggle")
