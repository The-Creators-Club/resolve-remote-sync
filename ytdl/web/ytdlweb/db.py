...
# without consulting user_version. Ordered by key. Empty on purpose: v1 is what
# schema.sql creates.
_MIGRATIONS = {}
...
def mark_pending(c, job_id):
    ...
    cur = c.execute(
        ...
        "AND dl_state IN ('none','failed','skipped')", (job_id,))
    c.commit()
    return cur.rowcount
...
def ledger_map(c):
    """{video_id: '<project label>/<term>'} -- what the ALREADY IN badge shows."""
    return {r['video_id']: f"{r['project_label']}/{r['term']}"
            for r in c.execute('SELECT video_id, project_label, term FROM downloads')}
...
def ledger_add(c, video_id, title, channel, project_slug, project_label, term,
    ...
    c.execute(
        ...
        'term,rel_path,job_id,downloaded_by,downloaded_at) VALUES(?,?,?,?,?,?,?,?,?,?) '
        ...
        (video_id, title, channel, project_slug, project_label, term, rel_path,
         job_id, downloaded_by, now()))
    c.commit()
