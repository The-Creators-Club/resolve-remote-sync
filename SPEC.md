...
- Auto-provisioning (added same day, after the "Creator Profiles" gap): the collector scans the Projects tree (read-only mount) every 5 min and creates + shares the Syncthing folder for any project dir lacking one, so new projects need only `setup_tree.py`. Trigger is deliberately the tree, not the Resolve project DB — a Resolve project name carries no year/series path to map from; a DB cross-check ("Resolve project with no media folder") is possible future work.
