...
def test_the_toast_clears_the_dock():
    assert "body:has(.hud-dock) .toast { bottom: calc(14px + var(--cc-dock-h, 0px)); }" in _text(SHEET)


def test_hud_common_comes_after_the_first_phone_block():
    """test_bug_hunt_2026_09_24_w2_music-ytdl.py reads the FIRST
    `@media (max-width: 600px)` block and needs `.search-bar > #q` in it."""
    css = _text(SHEET)
    first = css.index("@media (max-width: 600px)")
    assert first < css.index(BEGIN)
    assert ".search-bar > #q" in css[first:css.index("}", css.index(".search-bar > #q", first)) + 1]
