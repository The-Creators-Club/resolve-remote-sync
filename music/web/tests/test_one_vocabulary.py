"""One vocabulary: the retired words never come back into the music page's copy.
...
# Empty on purpose, and it should stay that way. The one phrase that would
# have earned a place, "a synthetic drum machine beat" in the CLAP vocabulary
# (musicweb/vocab.py), is out of this scan's scope: it is a tag description,
# not a `detail`, and "machine" there is half the name of a TR-808 rather than
# a word for a computer.
...
def test_the_error_sentinel_and_the_copy_moved_together() -> None:
    """app.js tells "the tray answered with an HTTP error" from "the request
    never reached 127.0.0.1" by the PREFIX of the thrown Error. Renaming the
    copy without the sentinel would silently pick the wrong sentence for every
    failure, which is the 2026-08-12 inversion all over again."""
    text = (STATIC / "app.js").read_text(encoding="utf-8")
    assert "throw new Error(`tray HTTP ${r.status}`)" in text
    assert text.count("startsWith('tray ')") == 2
    assert "startsWith('companion')" not in text
