...
docs/UI_REDESIGN_PORT_PLAN.md 1.2, 3.1, 5.1, 7.1 row 3, R15. Classic pins
are untouched: every test here runs under the `ui_variant` fixture and
asserts the classic shape when `everyday` is off and the terminal shape
when it is on.
"""
    ...
    with TestClient(app) as client:
        ...
        for editor, machine in (("owen", "OWEN-LAPTOP"), ("tchen", "TCHEN-PC")):
            ...
            db.add_selection(conn, editor, "2026-ff5-elections", "admin", now,
                             machine=machine)
        conn.commit()
        yield client, conn, ui_variant
        conn.close()


def _on(v) -> bool:
    return "everyday" in v.groups


def _hx(client, v, url="http://testserver/account"):
    return v.htmx_headers(client, url)


# ------------------------------------------------------------ static facts

def test_every_everyday_template_is_in_the_group_table():
    from ccsync_dashboard import ui_variant as uv
    for name in EVERYDAY_FILES:
        assert (CC / name).is_file(), name
        assert uv.TEMPLATE_GROUPS["cc/" + name] == "everyday", name
...
def test_the_terminal_account_script_asks_for_an_empty_answer():
    ...
    # the classic file is untouched
    classic = (DASH / "static" / "account.js").read_text(encoding="utf-8")
    assert "view=none" not in classic
...
def test_each_page_draws_in_the_look_it_is_asked_for(env, path):
    client, _conn, v = env
    ...
    if _on(v):
        ...
        assert "cc/everyday.css" not in page.text
...
def test_the_signed_out_sign_in_page(env):
    ...
    if _on(v):
        ...
        # the way back is there, and it works without a session
        assert "/ui/preview?variant=classic" in page.text
        assert 'class="hud' not in page.text
    else:
        assert "ev-gate" not in page.text
...
def test_transfers_poll_selects_ids_every_state_renders(env):
    ...
    frag = client.get("/partials/transfers",
                      headers=_hx(client, v, "http://testserver/transfers")).text
    for i in ids:
...
def test_the_account_page_keeps_every_classic_hook(env):
    ...
    if _on(v):
        ...
        assert "/static/account.js" in page


def test_account_partials_answer_in_the_asking_pages_look(env):
    client, _conn, v = env
    _cookie(client, "owen")
    h = _hx(client, v)
    ...
    if _on(v):
        ...
        assert "[ SIGNED-IN BROWSERS ]" in sess.text
        assert "[ SAVE ]" in name.text
...
def test_the_legal_gap_controls_reach_the_account_page(env):
    ...
    if _on(v):
        ...
        assert "[ IDLE TIME ]" in page and "[ DOWNLOAD ]" in page


def test_the_person_queue_is_terminal_only(env):
    client, _conn, v = env
    _cookie(client, "owen")
    resp = client.get("/partials/person-queue", headers=_hx(client, v))
    if _on(v):
