...
- **Coloured lamps on road infrastructure are not emergency vehicles.** Tunnels,
  toll plazas and gantries carry fixed red, amber, green and blue lamps — lane
  control signals, exit signs, height and clearance indicators. A real failure:
  a tunnel gantry's blue and amber lamps were repeatedly indexed as "emergency
  response vehicles with blue and red flashing lights"; at full resolution they
  are bolted to a structure and no emergency vehicle is present anywhere in the
  frame. Before writing "emergency vehicle", check you can see **the vehicle** —
  a body, wheels, livery — and not merely a coloured light. If you can only see
  a light, say so: `blue lamp on gantry`, not `police car`.
...
Report **one segment per distinct shot**. A shot is a run of *consecutive* cells showing
the same continuous scene — same place, same subject, same camera setup.

**Split whenever the picture changes.** If two adjacent cells show a different place, a
different subject, or different framing, they are two shots and need two segments. Merge
cells only when they are genuinely the same shot sampled more than once: frames are taken
every few seconds, so one long static shot legitimately occupies several neighbouring
cells and should be a single segment covering all of them.

**Do not average a sheet down into a summary.** A real failure from this archive. One 3x3
sheet held a petrol-station forecourt, a shopfront street, a narrow alley, a CNN webpage
screenshot, masked pedestrians in close-up, and a stockroom doorway — six different
things. It came back as three broad segments reading "urban street, scooters, buildings,
storefronts". The masks, the traffic cone and the pedestrian crossing were all plainly in
frame; none were recorded, so none can be found. Nine visibly different cells means nine
segments.

An editor uses this index to jump straight to an exact shot, so a segment spanning several
different shots lands them on the wrong frame. **When in doubt, split.** More segments with
tight timecodes are far more useful than fewer broad ones.
...
## Shots with no usable visuals: label them and move on

Some shots contain nothing an editor would ever search for visually. A studio
newsreader at a desk is the common case: nobody cuts to it for the *picture*.
Describing the anchor's jacket, the desk, the studio lighting and the background
graphics wastes effort and invites invention, because there is so little to say
that the temptation is to embroider.

When a segment is one of these, set `description` to the canonical label alone —
nothing else — and keep `objects` to the label plus its obvious synonyms:

| shot | `description` | `objects` |
|---|---|---|
| studio anchor / presenter to camera | `newsreader` | `newsreader, news anchor, presenter, news studio, talking head` |
| title card / programme opener | `title card` | `title card, opening titles, graphic` |
| black or blank frames | `black frame` | `black frame, blank` |
| colour bars / test pattern | `colour bars` | `colour bars, test pattern` |
| station ident / logo sting | `station ident` | `station ident, channel logo` |

Use the exact label spellings above so these are reliably findable — and just as
importantly, reliably *filterable* when an editor wants to exclude them.

Three things this rule does **not** cover:

- **Still capture `onscreen_text`.** A newsreader shot often carries the single
  most valuable text in the whole clip — the chyron naming the story, the people
  and the place. The picture is worthless; the text on it is not. Read it as
  carefully as anywhere else.
- **An interview or piece-to-camera on location is NOT a newsreader shot.** If
  the person is somewhere identifiable, or is a named subject rather than a
  studio presenter, describe it normally — who and where is searchable.
- **A newsreader shot with an inset window or over-the-shoulder graphic** is not
  a bare newsreader shot. Describe the inset content, which is real footage:
  `newsreader, inset window: harbour with fishing boats`.
