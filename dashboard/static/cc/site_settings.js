...
(function () {
  ...
  // Carried from the classic site_settings.js. The name a switch is asked
  // about by is its `data-label` (the classic page read the label's first
  // text node; here that text sits in span.l beside the hint).
  function telemetryBoxLabel(box) {
    return box.getAttribute("data-label") || box.getAttribute("data-telemetry");
  }

  function telemetrySwitchingOff(form) {
    var names = [];
    Array.prototype.forEach.call(form.querySelectorAll("input[data-telemetry]"), function (box) {
      if (box.getAttribute("data-initial") === "1" && !box.checked) names.push(telemetryBoxLabel(box));
    });
    return names;
  }

  // The one wording for "switching this off deletes data", shared by Save,
  // import and undo (LG-1 review round): all three reach the same fleet-wide
  // delete in set_many, so none may ask less plainly. Phone-dialog length.
  function telemetryOffQuestion(names) {
    return "Stop every computer reporting: " + names.join(", ") + "? " +
      "What this dashboard holds about " + (names.length === 1 ? "it" : "them") +
      " is deleted for every computer now, for good. Sync is not affected.";
  }

  // `keys` are the server's category names (telemetry_off).
  function telemetryOffWarning(keys) {
    ...
    return " " + telemetryOffQuestion(names);
  }

  function markTelemetrySaved(form) {
    Array.prototype.forEach.call(form.querySelectorAll("input[data-telemetry]"), function (box) {
      box.setAttribute("data-initial", box.checked ? "1" : "0");
    });
  }

  // LG-4 (docs/LEGAL_GAP_FEATURES_PLAN.md 4.4): information, never a
  // warning. None on plain http means no line at all; plain http from the
  // public internet is the alert's job, named here so the two agree.
  ...
  // ============================================== import, history, undo
  function importQuestion(changes, telemetryOff) {
  ...
  function runImport(text, result, key) {
    ...
    return api("/api/v1/admin/site/import?dry_run=1", {method: "POST", body: JSON.stringify({text: text})})
      .then(function (preview) {
        ...
        ask(importQuestion(preview.changes, preview.telemetry_off), "Apply", function () {
      ...
      if (undo) {
        ...
        undo.onclick = function () {
          ...
          ask("Put back the " + count + " setting" + (count === 1 ? "" : "s") + " changed by " + latest.actor + " " +
              agoText(latest.at) + " (" + latest.at + ")?" +
              // LG-1 review round: an undo reaches the same fleet-wide delete
              // a Save does (setup_routes._undo_telemetry_off).
              telemetryOffWarning(latest.telemetry_off), "Undo", function () {
  ...
  function wireSave() {
    ...
    form.addEventListener("submit", function (evt) {
      evt.preventDefault();
      // LG-1 (4.1): switching a reporting category OFF deletes what the
      // dashboard holds about it for every computer, so ask first, naming
      // what goes. Switching one back ON deletes nothing and is not asked.
      var switchingOff = telemetrySwitchingOff(form);
      if (switchingOff.length) {
        ask(telemetryOffQuestion(switchingOff), "Stop reporting", function () { saveNow(form); });
        return;
      }
      saveNow(form);
    });
