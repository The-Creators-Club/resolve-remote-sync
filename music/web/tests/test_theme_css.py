...
facts. This is the music copy of dashboard/tests/test_theme_css.py --
...
def test_the_filter_rail_headings_are_red_labels_over_a_hairline():
    """FEEL / TEMPO & LENGTH / GENRE / MOOD -- the rail in the owner's
    screenshot. red-dim, not the grey --border used for row separators."""
    body = _rule(".filter-rail h2")
    assert "color: var(--red)" in body
    assert "text-transform: uppercase" in body
    assert "border-bottom: 1px dotted var(--red-dim)" in body


def test_the_filter_rail_is_a_panel_with_its_own_scroller():
    """The rail scrolls independently of the results, which is why it is the
    surface the stock grey scrollbar was found on."""
    body = _rule(".filter-rail")
    assert "background: var(--panel)" in body
    assert "border-right: 1px solid var(--red-dim)" in body
    assert "overflow-y: auto" in body


def test_the_feel_sliders_stay_full_width_over_the_shared_paint():
